...
>> 위 경우들을 매번 다 구현해주면 끝이없음. 엄청나게 많이 만들어도 100% 커버리지가 안됨
함수를 일반화하거나, 재사용할수 있도록 해야함. 다양한 인풋 소스에 대해서 알아서 대응하도록 작성할수 있어야함 >> 리더 인터페이스


리더인터페이스를 구현하면 그냥 값을 받을수 있음. 일단 []bytes 로 받고 나머지 가공은 알아서 하렴

인터페이스 >> 타입이 다른데 동작이 비슷한거를 유연하게 구현할수 있도록
...
- 이거 한줄 이해하기
  ...
  - []byte >> Writer >> 출력체널/Source of Output(문자열이든, 바이너리든, 텍스트던, 터미널이던)
  - 여기서 Writer 인터페이스를 구현하는거임!!

- Reader 와 Writer 는 어떻게 연결 된걸까??

  - ?? 설명들어도 모르겠음

- `io.Copy` 함수는?
  - 도큐먼트 읽어보면 : `func Copy(dst Writer, src Reader)` 이라고 적혀있는데,
  - 파일의 형태로 통신한다! 이게 핵심인듯? >> C언어의 레슨런이랑 통하는게 있다

## 고루틴

- 프로세스 안에서 고루틴이 돌아간다
- `http.Get(link)` 이 함수는 Blocking Call 임
- 고루틴이 생성되면 바로 고루틴이 만들어지는게 아니라 블럭킹 함수 콜이 발생하면 그때서야 돌아가서
  - 규칙과 사이드이펙트가 있다, 이해해야할 내용도 많이 있고

### 고 스케쥴러

- cpu 당 하나의 Go Scheduler 1개 >> 고루틴 N 개
- 여러개를 실행해도 결국 한번에 하나씩만 실행하는게 고루틴임

### 동시성은 병렬성이 아니다!

- 동시성은 : (컨커런시) >> 하나의 시피유가 여러개의 고루틴(쓰래드)을 돌리는거
- 병렬성 : (페레럴리즘) >> 여러개의 씨피유가 동일한 순간에 고루틴(쓰레드들)을 동일한 순간에 여러개 돌리는걸 의미함
...
- 다른 루틴(메인루틴, 자식루틴) 에서 같은 변수를 참조하지 않음!
- 값을 넘길때마다 메모리에서 값을 복사해간다(아규먼트로 넘기면 무조건 복사한다!)

## 자바 JDK17 에서 버추얼쓰레드

- 그거도 체널이랑 볼튼? 뭐 그런게 있어야했던거같음
